...
constructor(props){
  ...
  render() {
    ...
    const allFilms = this.props.movies.films.map((item, index) => {
        return <BookRow handleDetailsClick={this.props.handleDetailsClick} film={item} key={index} isFave={this.props.faves.includes(item)} onFaveToggle={() =>this.props.onFaveToggle(item)}/>
    })
